...
const Portfolio = () => {
  ...
  useEffect(() => {
    ...
    const typingSpeed = isDeleting ? 50 : 100; // Menghapus lebih cepat daripada mengetik
  ...
  return (
    <div className="min-h-screen bg-gradient-dark-purple text-gray-100">
      ...
      <div className="fixed top-6 left-0 w-full z-50 flex justify-center px-4">
        <nav className="bg-white backdrop-blur-xl border border-white/10 rounded-full px-8 py-3 shadow-2xl transition-all duration-300 w-full max-w-[fit-content]">
            ...
            <div className="hidden md:flex items-center space-x-8">
              {navLinks.map((link) => (
                <button
                  ...
                  className={`text-sm font-medium text-gray-300 hover:text-white transition-all duration-300 ${activeSection === link.id ? "text-white" : ""
                    }`}
                >
                  {link.label}
                </button>
          ...
          {/* Mobile Menu (Inside pill or separate? Image shows desktop links) */}
          {/* We'll use a simple mobile expansion for now */}
          {isMenuOpen && (
            <div className="md:hidden mt-4 pb-2 flex flex-col items-center space-y-4 border-t border-white/10 pt-4">
              {navLinks.map((link) => (
                <button
                  key={link.id}
                  onClick={() => scrollToSection(link.id)}
